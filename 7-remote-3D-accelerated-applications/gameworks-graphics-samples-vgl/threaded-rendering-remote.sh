...
if [ -z ${REMOTE+x} ]; then
    echo "Usage: REMOTE=<display server container> ./threaded-rendering-remote.sh"
    echo "e.g. REMOTE=xserver-xspice ./threaded-rendering-remote.sh"
    exit 1
fi
...
# Launch ThreadedRenderingGL. Use --volumes-from to mount /tmp/.X11-unix
