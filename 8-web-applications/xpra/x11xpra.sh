...
$DOCKER_COMMAND run --rm -it -d \
    ...
    -e DISPLAY=:1 \
    xpra-html5 start --bind=$HOME/.xpra/xpra-socket --bind-tcp=0.0.0.0:10000
...
$DOCKER_COMMAND run --rm -it \
    --volumes-from x11xpra-html5 \
    xpra-html5 bash -c "while [ ! -f $XDG_RUNTIME_DIR/pulse/pid ]; do echo 'Waiting for xpra Pulseaudio daemon';sleep 1; done; echo 'xpra Pulseaudio daemon running'"

# Launch firefox. Use --volumes-from to mount /tmp/.X11-unix
# from x11xpra-html5 and also use that container's IPC
$DOCKER_COMMAND run --rm \
    ...
    -e DISPLAY=:1 \
    --ipc=container:x11xpra-html5 \
    --volumes-from x11xpra-html5 \
    firefox

$DOCKER_COMMAND stop x11xpra-html5
